...
from pi_trees_ros.pi_trees_ros2 import *
from pi_kobuki_msgs.msg import Dock
from pi_kobuki_tasks.task_setup import *
...
class BlackBoard():
    def __init__(self, id, name, fake):
        self.memory = {}
...
class Patrol():
    def __init__(self):
        ...
        self.blackboard = BlackBoard(1, "BlackBoard", fake=self.fake)
        ...
        WAIT30 = WaitTask("WAIT30", interval=30, debug=True)
        
        if self.fake:
            FAKE_CHARGING = FakeCharging("FAKE_CHARGING", self.blackboard, interval=3, announce=True)
            AUTO_DOCK = SimpleActionTask("AUTO_DOCK", "move_base", MoveBaseAction, dock_goal, reset_after=False)
            GOTO_DOCK = Sequence("GOTO_DOCK", [NAV_NEAR_DOCK, AUTO_DOCK], announce=True)
            ...
            RECHARGE = Sequence ("RECHARGE", [GOTO_DOCK, FAKE_CHARGING, RECHARGE_COMPLETE], reset_after=True)
            MONITOR_BATTERY = MonitorTask("MONITOR_BATTERY", "battery_level", Float32, self.monitor_fake_battery)
        else:
        ...
        if self.fake:
            ...
            GET_DIAGNOSTICS = CallbackTask("GET_DIAGNOSTICS", self.get_fake_diagnostics, announce=True)
        ...
        STAY_HEALTHY = Selector("STAY_HEALTHY", announce=True)
        ...
        with STAY_HEALTHY:
            ...
            RECHARGE_STATUS = Sequence("RECHARGE_STATUS", [CHECK_CHARGING, CHECK_CHARGE_COMPLETE], announce=True)

            # If we are not charging, is the battery level OK?
            CHECK_CHARGE = Selector("CHECK_CHARGE", [RECHARGE_STATUS, CHECK_BATTERY_OK], announce=True)
        ...
        DOCK_REQUEST = Sequence("DOCK_REQUEST", reset_after=False, announce=True)
        ...
        INVERT_GET_OFF_DOCK = Invert("OFF_DOCK?", GET_OFF_DOCK)
                
        # A sequence to handle a dock request
        HANDLE_DOCK_REQUEST = Selector("HANDLE_DOCK_REQUEST", reset_after=False)
        ...
        PATROL = Iterator("PATROL", reset_after=False, announce=True)
        ...
        LOOP_PATROL = Loop("LOOP_PATROL", PATROL, iterations=self.n_patrols)
        
        # Add the patrol to the loop decorator
        #LOOP_PATROL.add_child(PATROL)
        
        #LOOP_PATROL = Loop("LOOP_PATROL", PATROL)
...
#             self.blackboard.memory['on_dock = False
    ...
    def get_fake_diagnostics(self):
        # Code taken from kobuki_dashboard package
        if self.fake:
            #self.blackboard.memory['laptop_battery_level'] = 100
            self.blackboard.memory['laptop_charging'] = False
            #self.blackboard.memory['robot_low_battery_alert'] = False
    ...
    def check_off_dock_cb(self):
        ...
        if self.fake:
            return not self.blackboard.memory['on_dock']
        else:
            if self.blackboard.memory['robot_charging'] is None and self.blackboard.memory['on_dock'] is None:
                return
            elif self.blackboard.memory['robot_charging'] or self.blackboard.memory['on_dock']:
    ...
    def check_on_dock_cb(self):
        ...
        if self.fake:
            return self.blackboard.memory['on_dock']
        else:
            if self.blackboard.memory['robot_charging'] is None and self.blackboard.memory['on_dock'] is None:
                return
            elif self.blackboard.memory['robot_charging'] or self.blackboard.memory['on_dock']:
    ...
    def check_battery_cb(self):
        ...
        if self.blackboard.memory['robot_battery_level'] is None:
            return
        else:
    ...
    def check_charge_complete_cb(self):
        rospy.logdebug("WAITING FOR CHARGING TO COMPLETE.  CHARGE AT " + str(int(self.blackboard.memory['robot_battery_level'])) + "%")
        
        if self.blackboard.memory['robot_battery_level'] is None:
            return
        else:
            if self.blackboard.memory['robot_battery_level'] < self.full_charge:
                return False
            else:
    ...
    def check_requested_dock_complete(self):
        ...
        if self.blackboard.memory['requested_dock_complete'] is None:
            return None
        elif self.blackboard.memory['requested_dock_complete']:
            return True
        else:
             return False
    ...
    def monitor_fake_battery(self, msg):
        ...
        self.blackboard.memory['robot_battery_level'] = msg.data
        return True
    ...
    def autodock_feedback_cb(self, feedback):
        ...
        if feedback.state == 'DOCKED_IN' and not self.blackboard.memory['on_dock']:
            self.blackboard.memory['on_dock'] = True
...
class BackOffDock(Action):
    def __init__(self, name, blackboard, cmd_vel_pub, interval=6):
...
class FakeCharging(Action):
    ...
    def run(self):
        ...
        if self.status != TaskStatus.SUCCESS:
            rospy.loginfo("RECHARGING THE ROBOT...")
            self.blackboard.memory['on_dock'] = True
            self.timer += 0.1
            rospy.sleep(0.1)
...
class SetVariable(Action):
    ...
    def run(self):
        self.blackboard.memory['self.variable'] = self.value
